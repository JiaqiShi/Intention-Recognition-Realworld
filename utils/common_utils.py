...
def intention_inference(pose_2d_dict,
                        im0,
                        model_pos,
                        res_model,
                        intent_model=None,
    ...
    processed_2d_pose = process_2dpose_for_3d(pose_2d_dict,
                                              norm_pose_2d=False,
    ...
    if intent_model is not None:
        ...
        if len(prediction_3d.shape) == 3:
            input_3d_pose = [
                np.pad(prediction_3d,
                       ((0, max_len - len(prediction_3d)), (0, 0), (0, 0))),
                len(prediction_3d)
            ]
        elif len(prediction_3d.shape) == 2:
            input_3d_pose = [
                np.pad(prediction_3d,
                       ((0, max_len - len(prediction_3d)), (0, 0))),
                len(prediction_3d)
            ]
        ...
        inputs = [
            torch.tensor(d).unsqueeze(0).to(device) if type(d) != list else
            [torch.tensor(e).unsqueeze(0).to(device) for e in d]
            for d in [input_3d_pose, res_features_cut, res_features_all]
        ]
...
def process_2dpose_for_3d(pose_2d_dict,
                          norm_pose_2d=False,
                          normalize_screen=True,
                          res_w=None,
                          res_h=None):
    ...
    if pose_2d_dict['keypoints'][0].shape[1] == 3:
        pose_2d_dict['keypoints'] = [
            pose[:, :2] for pose in pose_2d_dict['keypoints']
        ]
    ...
    if norm_pose_2d:
        ...
        def norm_pose_2d_to_same_scale(pose_2d_dict, bbox_center, bbox_scale):
            kpts_norm = []
            for kpt_frame, bbox_frame in zip(pose_2d_dict['keypoints'],
                                             pose_2d_dict['bbox']):
                ...
                kpts_norm.append((kpt_frame[:, :2] - center) / scale *
                                 bbox_scale + bbox_center)
            pose_2d_dict['keypoints'] = np.array(kpts_norm)

            return pose_2d_dict

        pose_2d_dict = norm_pose_2d_to_same_scale(pose_2d_dict, bbox_center,
                                                  bbox_scale)
...
def update_imgs_list(imgs_list, person_tracking_dict, img, max_length=750):
    ...
    if len(person_tracking_dict) == 0:
        ...
        too_long_ids = [
            pid for pid, person_dict in person_tracking_dict.items()
            if len(person_dict['bbox']) > max_length
        ]
        tracking_lengths = [
            len(person_dict['bbox'])
            for person_dict in person_tracking_dict.values()
            if len(person_dict['bbox']) <= max_length
        ]
...
def pick_up_target(person_tracking_dict, crop_paras, tracked_ids=[]):
    ...
    for track_id, singleperson_track_dict in person_tracking_dict.items():
        ...
        start_above_line = is_above_line((bbox_start[0] + bbox_start[2]) / 2,
                                         bbox_start[3], A, B, C)
        if not start_above_line:
            bbox_end = singleperson_track_dict['bbox'][-1]
            end_above_line = is_above_line((bbox_end[0] + bbox_end[2]) / 2,
                                           bbox_end[3], A, B, C)
            if end_above_line:
                tracked_ids.append(track_id)
                if len(singleperson_track_dict['bbox']) > 20 and len(
                        singleperson_track_dict['bbox']) < 600:
                    return singleperson_track_dict.copy(
                    ), track_id, tracked_ids
    return None, None, tracked_ids


def save_result_func(tracking_id,
                     saving_dir,
                     save_path,
                     save_images=None,
                     fps=29.97,
        ...
        if save_images is not None:
            for frame_idx, img in enumerate(save_images):
                if frame_idx == 0:
                    ...
                    video_file_path = os.path.join(save_path,
                                                   str(tracking_id) + '.mp4')
                    vid_writer = cv2.VideoWriter(
                        video_file_path, cv2.VideoWriter_fourcc(*'mp4v'), fps,
                        (w, h))
